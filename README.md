...
- Firestore role gate via `users/{uid}.role == "admin"` for data access.
...
- Member fields:
  ...
  - Start date
  - Expiry date
- Start date uses date picker UX; expiry is auto-calculated from plan duration.
...
9. Add `users/{uid}` document for that account with:
   ```json
   { "role": "admin" }
   ```
